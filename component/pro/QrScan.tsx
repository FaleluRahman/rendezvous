...
export default function QrScan({ setScan }: QrScanProps) {
  ...
  const collect = async () => {
    ...
    try {
      ...
      // Parse the QR data to extract event and timestamp
      ...
      // Make POST request as expected by PHP backend
      const response = await axios.post(
        `https://rend-application.abaqas.in/qrscans/actions.php?api=b1daf1bbc7bbd214045af&event=${eventId}&student=${jamiaId}&t=${timestamp}`,
  ...
  useEffect(() => {
    ...
    console.log("QR Data scanned:", data); // Debug log
  ...
  const handleScanResult = (result: any) => {
    try {
      if (result?.[0]?.rawValue) {
        ...
        console.log("Raw QR data:", scannedData); // Debug log
        setData(scannedData);
      }
    } catch (error) {
