...
//       axiosInstance
//         .post("/login", { jamiaId: scanResult }) // now sending plain text
//         .then((res) => {
//           if (res?.data?.status) {
//             Cookies.set("student", JSON.stringify(res.data.student), {
//               path: "/",
...
const QrCodeScanner = () => {
  ...
  return (
    <div className="bg-gray-50 flex items-center justify-center flex-col py-10 min-h-screen text-center">
      ...
      {error && !loading && (
        <div className="bg-white rounded-3xl shadow-lg p-6 text-center max-w-md w-full mx-4 mt-4">
          <p className="text-red-600 flex items-center justify-center mb-4">
            {error}
            <MdError className="mt-1 ml-2" />
          </p>
          <button
            onClick={() => setError(null)}
            className="bg-red-600 py-2 px-4 w-fit font-bold hover:bg-red-700 rounded-xl text-white transition-colors"
          >
            Scan Again
          </button>
        </div>
